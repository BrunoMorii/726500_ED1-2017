...
int inserirTerritorio(Territorio raiz, char lado, int importancia);
					...
					if(pai != NULL){
						if((lado == 'E' && pai->esq != NULL) || (lado == 'D' && pai->dir != NULL)){
							...
							if(ter != NULL){
								...
								if(verificaABBT(novo->raiz) == 0 && novo->importancia > 0){
									novo->importancia *= -1;
								}else if(verificaABBT(novo->raiz) == 1 && novo->importancia < 0){
									novo->importancia *= -1;
								}

								if(novo->importancia < 0){
									novo->importancia -= importancia;
								}else{
									novo->importancia += importancia;
								}
								
								if(!verificaABBC(raiz)){
									rem = buscaCasaG(raiz, novo->importancia);
									rem = removeCasaG(&rem);
									novo = buscaCasa(raiz, novo->importancia);
									if(novo == NULL){
										rem->esq = NULL;
										rem->dir = NULL;
...
Casa buscaCasaG(Casa raiz, int importancia){
	...
	if(raiz != NULL){
		...
		p = buscaCasaG(raiz->esq, importancia);
		if(p != NULL)
			return p;

		p = buscaCasaG(raiz->dir, importancia);
		if(p != NULL)
			return p;
	}
...
int removeTerritorio(Territorio *no, int importancia){
	...
	if(*no == NULL){
		return 0;
	}else if((*no)->valor > importancia){
		return removeTerritorio(&(*no)->esq, importancia);
	}else if((*no)->valor < importancia){
		return removeTerritorio(&(*no)->dir, importancia);
	}else{
		q = *no;
		if(q->esq == NULL){
			*no = q->dir;
